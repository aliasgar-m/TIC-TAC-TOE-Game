...
class Singleplayer:
    def __init__(self, player = 'x', computer = 'o'):
        ...
        self.agent = Agent(env=self.env, sym=2, pre_trained=True)
        self.player = player
    ...
    def load_q_values(self, filename='Q_values.txt') -> dict:
        with open(filename, "rb") as value_dict:
            dict_ = pickle.load(value_dict)
            return dict_
